...
- Object(객체): 변수(데이터) 들과 그와 관련된 메서드(함수)들이 모여서 이룬 하나의 꾸러미.
  [참고] JAVA는 객체 지향부터 시작하는 언어이다.
*/
...
{
/**************** Topic 1 ****************/
	string name = "Jack Jack"; // <-- 하나하나를 member variable이라고 부른다.
...
/**************** Topic 3 ****************/
	...
	// --> Struct를 이용해도 동일한 친구가 추가되면 print(so.name, so.address, ...)를 해야 한다.
